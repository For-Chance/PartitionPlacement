...
namespace Partition
{
	...
	private:
		...
		/// <returns></returns>
		Polygon_2 get_Poly_from_Face(const Face_handle& face) const {
		...
		std::vector<Vertex_handle> get_adj_centerline_vertex(
			const std::unordered_set<Vertex_handle>& centerline_vertices,
			...
			do {
				...
				if (h->is_bisector() && centerline_vertices.find(adj) != centerline_vertices.end()) {
					adj_vertex.push_back(adj);
				}
				h = h->next()->opposite();
			} while (h != v->halfedge());
			return std::move(adj_vertex);
		};
		std::vector<Halfedge_handle> get_adj_centerline_halfedge(
		...
		void parse_skeleton_to_centerline(
					...
					*/
					if (from->id() < to->id()) {
						...
						if (is_ans)
						{
							...
							centerline_vertices.insert(from);
							centerline_vertices.insert(to);
			...
			auto get_next_ans_vertex = [&](
				const std::unordered_set<Vertex_handle>& centerline_vertices,
				const Vertex_handle& v
				) {
					std::vector<Vertex_handle> adj_vertices = get_adj_centerline_vertex(centerline_vertices, v);
			...
			for (std::pair<Vertex_handle, bool>& it : he_pool)
				if (it.second == true)   // is centerline candidate
					if (centerline_vertex2cnt[it.first] == 1)
						leaf_vertices.insert(it.first);
			for (Vertex_handle lv : leaf_vertices) {
				Vertex_handle& cur_v = lv;
				bool delete_v = true;
				do {
					Vertex_handle next_v = get_next_ans_vertex(centerline_vertices, cur_v);
					if (next_v == nullptr)
						break;
			...
			for (auto& it : centerline_vertex2cnt)
				it.second = get_adj_centerline_vertex(centerline_vertices, it.first).size();
			for (auto& it : centerline_vertex2cnt)
				if (it.second == 3)
					connected_vertices.insert(it.first);
		}

		// assign 
		void assign_skeleton_segments(const boost::shared_ptr<Ss>& skeleton, std::vector<Segment_2>& skeleton_segments) {
			skeleton_segments.clear();
			for (Halfedge_iterator it = this->skeleton->halfedges_begin(); it != this->skeleton->halfedges_end(); ++it) {
				Vertex_handle from = it->opposite()->vertex(), to = it->vertex();
				skeleton_segments.push_back(Segment_2(from->point(), to->point()));
			}
		}
		void assign_skeleton_faces(const boost::shared_ptr<Ss>& skeleton, std::vector<Polygon_2>& skeleton_faces) {
			skeleton_faces.clear();
		...
		void assign_skeleton_centerlines(const boost::shared_ptr<Ss>& skeleton, const std::unordered_set<Halfedge_handle>& centerline_hes, std::vector<Segment_2>& skeleton_centerlines, std::vector<Segment_2>& other_centerlines) {
			skeleton_centerlines.clear();
			skeleton_otherlines.clear();
			for (Halfedge_iterator it = this->skeleton->halfedges_begin(); it != this->skeleton->halfedges_end(); ++it) {
				Vertex_handle from = it->opposite()->vertex(), to = it->vertex();
				if (from->id() < to->id()) {
					if (centerline_hes.find(it))
						skeleton_centerlines.push_back(Segment_2(from->point(), to->point()));
					else
						skeleton_otherlines.push_back(Segment_2(from->point(), to->point()));
				}
			}
		}
...
namespace Partition
{
	...
	Solver<K>::Solver(const CGAL::Polygon_with_holes_2<K>& space, const PartitionProps& props)
	{
		...
		if (props.withSimplifyBoundary) {
			...
			std::unordered_set<Vertex_handle> connected_vertices;
			std::unordered_set<Vertex_handle> leaf_vertices;
			parse_skeleton_to_centerline(this->skeleton, centerline_hes, centerline_vertices, connected_vertices, leaf_vertices);
